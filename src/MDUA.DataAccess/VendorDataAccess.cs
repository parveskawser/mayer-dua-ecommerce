...
namespace MDUA.DataAccess
{
    public partial class VendorDataAccess : IVendorDataAccess
    {
        ...
        public List<dynamic> GetPendingBills(int vendorId)
        {
            ...
            const string sql = "GetUnpaidPoReceivedByVendor"; // Matches the SP created earlier
            ...
            using (var cmd = GetSPCommand(sql))
            {
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        // Mapping the result from SP to dynamic object
                        list.Add(new
                        {
                            Id = reader["Id"],
                            InvoiceNo = reader["InvoiceNo"].ToString(),
                            // Safe casting ensures no runtime errors
                            ...
                            TotalAmount = reader["TotalPaymentDue"] // Optional: if you want to show total original amount
                        });
                    }
                }
            }
