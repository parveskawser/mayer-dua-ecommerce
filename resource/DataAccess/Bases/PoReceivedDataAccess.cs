...
{
	public partial class PoReceivedDataAccess : BaseDataAccess
	{
		#region Constants
		...
		private const string GETPORECEIVEDBYVENDORID = "GetPoReceivedByVendorId";
		...
		/// <summary>
        /// Retrieves all PoReceived objects by VendorId
        /// </summary>
        /// <returns>A list of PoReceived objects</returns>
		public PoReceivedList GetByVendorId(Int32 _VendorId)
		{
			using( SqlCommand cmd = GetSPCommand(GETPORECEIVEDBYVENDORID))
			{
				
				AddParameter( cmd, pInt32(PoReceivedBase.Property_VendorId, _VendorId));
				return GetList(cmd, ALL_AVAILABLE_RECORDS);
			}
		}
		...
		protected void FillObject(PoReceivedBase poReceivedObject, SqlDataReader reader, int start)
		{
				...
				poReceivedObject.InvoiceNo = reader.GetString( start + 10 );			
