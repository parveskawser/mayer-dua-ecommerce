...
namespace MDUA.Facade
{
    public class OrderFacade : IOrderFacade
    {
        ...
        public async Task<string> PlaceGuestOrder(SalesOrderHeader orderData)
        {
            ...
            // Fallback if list is empty but single fields are used (Legacy support)
            ...
            // ==============================================================================
            // 3. CALCULATE TOTALS, VALIDATE STOCK & SECURITY CHECK
            // ==============================================================================
            int? companyId = null;
            ...
            {
                // ✅ Security Check: Verify Variant & Product exist
                ...
                // ✅ Security Check: Ensure all items belong to the same company
                ...
                // ✅ Stock Check
                if (variant.StockQty == 0) throw new Exception($"Item '{variant.VariantName}' is out of stock.");
                if (variant.StockQty < item.OrderQuantity) throw new Exception($"Insufficient stock for {variant.VariantName}. Available: {variant.StockQty}");
            ...
            // 4. SET HEADER TOTALS
            // Override frontend company data with real owner ID (Security Fix 1)
            ...
            using (SqlConnection connection = new SqlConnection(connStr))
            {
                ...
                using (SqlTransaction transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (customer == null)
                        {
                            // EMAIL DUPLICATE CHECK (only block if email belongs to another phone)
                            if (!string.IsNullOrEmpty(orderData.CustomerEmail))
                            {
                                var emailOwner = transCustomerDA.GetByEmail(orderData.CustomerEmail);
                                if (emailOwner != null && emailOwner.Phone != orderData.CustomerPhone)
                        ...
                        // ==============================================================================
                        // ✅ F. NOTIFICATIONS (Restored from V1)
                        // ==============================================================================
                        ...
                        try
                        {
                            ...
                            _ = Task.Run(async () => {
                                try
                                {
                                    ...
                                    {"OrderId", orderNo},
                                    {"OrderQty", items.Sum(x => x.OrderQuantity)}, // Sum of all items
