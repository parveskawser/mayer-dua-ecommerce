...
namespace MDUA.Facade
{
    public class ProductFacade : IProductFacade
    {
        ...
        public List<object> GetVariantStocks(List<int> variantIds)
        {
            var dict = _variantPriceStockDataAccess.GetStocksByVariantIds(variantIds);

            // return a JSON-friendly shape
            return dict.Select(kv => new { variantId = kv.Key, stockQty = kv.Value }).Cast<object>().ToList();
        }
