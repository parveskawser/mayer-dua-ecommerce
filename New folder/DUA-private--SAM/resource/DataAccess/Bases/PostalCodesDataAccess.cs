...
{
	public partial class PostalCodesDataAccess : BaseDataAccess
	{
		#region Constants
		...
		protected void FillObject(PostalCodesBase postalCodesObject, SqlDataReader reader, int start)
		{
				...
				if(!reader.IsDBNull(1)) postalCodesObject.PostCode = reader.GetString( start + 1 );			
				if(!reader.IsDBNull(2)) postalCodesObject.SubOfficeEn = reader.GetString( start + 2 );			
				if(!reader.IsDBNull(3)) postalCodesObject.SubOfficeBn = reader.GetString( start + 3 );			
				if(!reader.IsDBNull(4)) postalCodesObject.ThanaEn = reader.GetString( start + 4 );			
				if(!reader.IsDBNull(5)) postalCodesObject.ThanaBn = reader.GetString( start + 5 );			
				if(!reader.IsDBNull(6)) postalCodesObject.DistrictBn = reader.GetString( start + 6 );			
				if(!reader.IsDBNull(7)) postalCodesObject.DistrictEn = reader.GetString( start + 7 );			
				if(!reader.IsDBNull(8)) postalCodesObject.DivisionBn = reader.GetString( start + 8 );			
				if(!reader.IsDBNull(9)) postalCodesObject.DivisionEn = reader.GetString( start + 9 );			
