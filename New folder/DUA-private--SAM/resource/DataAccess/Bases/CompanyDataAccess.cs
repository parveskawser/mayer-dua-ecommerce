...
namespace MDUA.DataAccess
{
	public partial class CompanyDataAccess : BaseDataAccess
	{
		...
		private const string GETCOMPANYBYTHEMEID = "GetCompanyByThemeId";
		...
		{	
			...
			AddParameter(cmd, pInt32(CompanyBase.Property_ThemeId, companyObject.ThemeId));
			AddParameter(cmd, pNVarChar(CompanyBase.Property_ThemeSettings, companyObject.ThemeSettings));
		}
		#endregion
		...
		/// <summary>
        /// Retrieves all Company objects by ThemeId
        /// </summary>
        /// <returns>A list of Company objects</returns>
		public CompanyList GetByThemeId(Nullable<Int32> _ThemeId)
		{
			using( SqlCommand cmd = GetSPCommand(GETCOMPANYBYTHEMEID))
			{
				
				AddParameter( cmd, pInt32(CompanyBase.Property_ThemeId, _ThemeId));
				return GetList(cmd, ALL_AVAILABLE_RECORDS);
			}
		}
		...
		protected void FillObject(CompanyBase companyObject, SqlDataReader reader, int start)
		{
				...
				if(!reader.IsDBNull(13)) companyObject.ThemeId = reader.GetInt32( start + 13 );			
				if(!reader.IsDBNull(14)) companyObject.ThemeSettings = reader.GetString( start + 14 );			
			FillBaseObject(companyObject, reader, (start + 15));
