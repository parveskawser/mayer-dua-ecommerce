...
{
	public partial class AddressDataAccess : BaseDataAccess
	{
		#region Constants
		...
		{	
			...
			AddParameter(cmd, pNVarChar(AddressBase.Property_Thana, 100, addressObject.Thana));
			AddParameter(cmd, pNVarChar(AddressBase.Property_SubOffice, 100, addressObject.SubOffice));
		...
		protected void FillObject(AddressBase addressObject, SqlDataReader reader, int start)
		{
				...
				addressObject.Id = reader.GetInt32( start + 0 );			
				addressObject.CustomerId = reader.GetInt32( start + 1 );			
				if(!reader.IsDBNull(2)) addressObject.Country = reader.GetString( start + 2 );			
				if(!reader.IsDBNull(3)) addressObject.Divison = reader.GetString( start + 3 );			
				addressObject.City = reader.GetString( start + 4 );			
				if(!reader.IsDBNull(5)) addressObject.Thana = reader.GetString( start + 5 );			
				if(!reader.IsDBNull(6)) addressObject.SubOffice = reader.GetString( start + 6 );			
				addressObject.Street = reader.GetString( start + 7 );			
				addressObject.PostalCode = reader.GetString( start + 8 );			
				addressObject.ZipCode = reader.GetString( start + 9 ).ToCharArray();			
				if(!reader.IsDBNull(10)) addressObject.AddressType = reader.GetString( start + 10 );			
				addressObject.CreatedBy = reader.GetString( start + 11 );			
				addressObject.CreatedAt = reader.GetDateTime( start + 12 );			
				if(!reader.IsDBNull(13)) addressObject.UpdatedBy = reader.GetString( start + 13 );			
				if(!reader.IsDBNull(14)) addressObject.UpdatedAt = reader.GetDateTime( start + 14 );			
			FillBaseObject(addressObject, reader, (start + 15));
